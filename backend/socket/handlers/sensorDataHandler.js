...
const sensorDataHandler = (io, topic, message) => {
  ...
  switch (sensorType) {
    ...
    case "clean-water":
      return handleCleanWater(io, topicParts, message);
...
/**
 * Handle clean water level sensor data (module-7)
 * Format: smartcamper/sensors/clean-water/level
 */
function handleCleanWater(io, topicParts, message) {
  if (topicParts.length >= 4 && topicParts[3] === "level") {
    const value = parseFloat(message);
    ...
    console.log(
      `⚠️ Invalid clean water level value: ${message} (must be 0-100)`,
    );
    return true;
  }
...
/**
 * Handle toilet urine level sensor data (module-5)
...
function handleFloorHeating(io, topicParts, message) {
  ...
  if (topicParts.length >= 4 && topicParts[3] === "status") {
    try {
      ...
      console.log(
        `❌ Failed to parse floor heating status JSON: ${error.message}`,
      );
      return true; // Handled, but error
    }
  }
...
/**
 * Handle Victron energy status data
 * Format: smartcamper/sensors/module-6/status (JSON)
 */
function handleVictron(io, topicParts, message) {
  if (topicParts.length >= 4 && topicParts[3] === "status") {
    try {
      const statusData = JSON.parse(message);
      ...
      if (
        ...
        console.log("❌ Invalid Victron status JSON: expected object");
        return true;
      }

      io.emit("victronStatusUpdate", {
        type: "full",
        data: statusData,
        timestamp: new Date().toISOString(),
      });
