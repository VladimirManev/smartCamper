...
CommandHandler::CommandHandler(MQTTManager* mqtt, FloorHeatingManager* floorHeating, String moduleId) {
  ...
  if (floorHeating == nullptr) {
    if (DEBUG_SERIAL) {
      Serial.println("❌ ERROR: CommandHandler: floorHeatingManager cannot be nullptr!");
    }
  }
  ...
  this->floorHeatingManager = floorHeating;
...
void CommandHandler::forceUpdate() {
  ...
  // Call force update in FloorHeatingManager (validate pointer first)
  if (floorHeatingManager != nullptr) {
    floorHeatingManager->handleForceUpdate();
  } else {
    if (DEBUG_SERIAL) {
      Serial.println("❌ ERROR: Cannot force update - floorHeatingManager is nullptr");
    }
  }
}
...
void CommandHandler::printStatus() const {
  if (DEBUG_SERIAL) {
    ...
    Serial.println("  Floor Heating Manager: " + String(floorHeatingManager != nullptr ? "OK" : "NULL"));
  }
}
