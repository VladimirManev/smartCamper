...
| **Leveling LED** (Обратна връзка)    | 16  | Изход          | Визуална обратна връзка (3 бързи мигания) |
...
- Измерване: burst от 3 четения (~3s) → медиана от валидните (нужни ≥2 в диапазон 5–50°C)
- Интервал: на всеки 30 секунди докато кръгът е ON; също веднага при включване и след смяна на реле
...
- **Интервал на измерване**: 30 секунди (burst + медиана)
- **Settle след реле**: 1 секунда пауза на всички сензори (EMI)
- **OFF режим**: без измерване; температурата в статуса е `null` (без стара стойност)
...
- **Обратна връзка:** Leveling LED (GPIO 16) мига 3 пъти + сериално съобщение
...
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `smartcamper/sensors/module-3/status`        | `{"type": "full", "data": {"circles": {...}}}` или `{"type": "circle", "index": 0, "mode": "TEMP_CONTROL", "relay": "ON", "temperature": 32, "error": false}` | При промяна (бутон, MQTT, реле) или когато новата температура (медиана) се различава от последно изпратената (~30 s) |
...
- Температура: неблокиращ burst от 3 DS18B20 четения → медиана (нужни ≥2 валидни в 5–50°C)
- Burst на всеки 30 секунди; също веднага при включване на кръга и след всяко реле ON/OFF
- Автоматично управление на релето:
  - Реле ON когато температура < 32°C
  - Реле OFF когато температура >= 33°C
- Първо решение за реле след първия успешен burst (~3s след включване + settle ако трябва)
- Работи напълно офлайн (не се изисква мрежа)
- Бутонът превключва към OFF режим

### Обработка на грешки

- Ако 3 последователни burst-а се провалят (липсва сензор, `-127`, или &lt;2 валидни sample-а), кръгът се гаси (safe-off)
- Грешката се публикува в topic `smartcamper/errors/module-3/circle/{index}` (веднъж при възникване)
- В OFF режим температурата се нулира (`null` в статуса) — UI не показва стара стойност
...
- **FloorHeatingSensor**: DS18B20 burst/медиана (async state machine)
...
- **Неблокиращо четене**: Burst state machine (3× ~800ms конверсии)
- **Свежо четене при ON / смяна на реле**: Без стара температура след включване
- **Няма блокиращи забавяния**: Всички операции са неблокиращи за отзивчиви бутони
- **Оптимизиран debouncing**: 100ms debounce с 300ms минимален интервал между натискания
