...
CommandHandler::CommandHandler(MQTTManager* mqtt, ApplianceManager* appMgr, String moduleId) {
  ...
  if (appMgr == nullptr) {
    if (DEBUG_SERIAL) {
      Serial.println("❌ ERROR: CommandHandler: applianceManager cannot be nullptr!");
    }
  }
...
void CommandHandler::handleMQTTMessage(char* topic, byte* payload, unsigned int length) {
  ...
  // Other commands are handled by ApplianceManager::handleMQTTMessage
}

void CommandHandler::forceUpdate() {
  lastForceUpdate = millis();
  
  // Use ApplianceManager for force update
  if (applianceManager != nullptr) {
    applianceManager->handleForceUpdate();
  } else {
    if (DEBUG_SERIAL) {
      Serial.println("❌ ERROR: Cannot force update - ApplianceManager not available");
    }
  }
}
...
void CommandHandler::printStatus() const {
  if (DEBUG_SERIAL) {
    ...
    Serial.println("  Appliance Manager: " + String(applianceManager != nullptr ? "OK" : "NULL"));
  }
}
