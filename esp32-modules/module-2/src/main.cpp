/**
 * @file main.cpp
 * @brief Module 2 - Main Entry Point
 * 
 * ESP32 module for LED strip control with multi-strip support.
 * 
 * Architecture:
 * - ModuleManager: Handles common infrastructure (Network, MQTT, Heartbeat, Commands)
 * - LEDManager: Coordinates all LED-related functionality
 * - LEDStripController: Manages LED strips (control, transitions, dimming)
 * - ButtonHandler: Handles button input processing
 * - RelayController: Manages relay control
 * - PIRSensorHandler: Handles PIR motion sensor for automatic strip control
 */

#include "Config.h"
#include "ModuleManager.h"
#include "LEDManager.h"

// Global managers - initialized in setup()
ModuleManager moduleManager;
LEDManager ledManager(&moduleManager);

/**
 * @brief Setup function - called once on boot
 * 
 * Initializes:
 * 1. Module infrastructure (Network, MQTT, Heartbeat, Commands)
 * 2. LED-related components (strips, buttons, relays, PIR sensor)
 */
void setup() {
  // Initialize module infrastructure (Network, MQTT, Heartbeat, Commands)
  // CommandHandler is passed to ModuleManager for MQTT command processing
  // Note: LEDManager will set its own MQTT callback in begin() for LED-specific commands
  moduleManager.begin(&ledManager.getCommandHandler());
  
  // Verify initialization
  if (!moduleManager.isInitialized()) {
    if (DEBUG_SERIAL) {
      Serial.println("❌ ERROR: ModuleManager failed to initialize!");
    }
    // In production, could trigger watchdog reset here
    return;
  }
  
  // Initialize LED components
  ledManager.begin();
  
  if (DEBUG_SERIAL) {
    Serial.println("✅ Module 2 fully initialized and ready!");
    Serial.println("Click: Toggle strip ON/OFF (with random transitions)");
    Serial.println("Hold: Dim/Increase brightness\n");
  }
}

/**
 * @brief Main loop - called repeatedly
 * 
 * Update order is important:
 * 1. Infrastructure first (Network, MQTT, Heartbeat)
 * 2. LED components second (strips, buttons, PIR sensor)
 */
void loop() {
  // Update infrastructure (Network, MQTT, Heartbeat)
  // This must be called first to ensure connectivity
  moduleManager.loop();
  
  // ALWAYS update LED components (strips, buttons, PIR sensor) - physical functions must work offline
  // Status publishing will only happen if connected (handled inside LEDManager)
  ledManager.loop();
  
  delay(10);  // Small delay to prevent overwhelming the system
}
