...
| **Strip 5** (Спалня разширение)  | 23  | Изход          | 6 LED-а, RGBW (синхронизирана с Strip 4)             |
...
| **Power Relay**                  | 32  | Изход          | Управление на захранването на LED лентите (10A реле) |
...
| **5** | 6     | WS2815 RGBW | RGBW     | Автоматично (синхронизирана с Strip 4) |
...
**Яркост на банята (MQTT / приложение):** Няма физическо димиране с **задържан бутон** за тази лента, но `strip/3/brightness` ползва същия **плавен преход** като останалите, когато лентата е **ON** (в т.ч. **AUTO**, докато вече свети от PIR). След **завършен** плавен MQTT преход в режим **AUTO** firmware обновява `lastAutoBrightness`, за да е следващото включване от движение на това ниво.

**Публикуване на статус след `brightness`:** Пълният JSON на `smartcamper/sensors/module-2/status` се изпраща при **край на плавния преход**, не в момента на командата (избягва се остаряла `brightness` в JSON по време на fade).
...
| ------------------------------------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| `smartcamper/sensors/module-2/status` | `{"strips": {...}, "relays": {...}}`                                         | При промяна (след транзиции където е уместно; MQTT `brightness` след край на fade) |
...
| `smartcamper/commands/module-2/strip/{index}/brightness` | `{"value": 1-255}`              | Плавна яркост (clamp в firmware) |
...
### Power Relay Управление

Модулът включва power relay (GPIO 32 — виж `Config.h`), който физически прекъсва захранването на всички LED ленти, когато всички са изключени. Това предотвратява консумацията на ток в standby режим (над 0.5A) от WS2812 контролерите.

**Как работи:**

- Релето се включва автоматично когато някоя лента се включи (с 100ms забавяне за стабилизация)
- След delay-а всички ленти се нулират (черно), после стартира анимацията на желаната — предотвратява power-on glitch (святкане на първия диод)
- Релето се изключва автоматично когато всички ленти са изключени (safety проверка на всеки 60 секунди)
- Предотвратява изразходване на батерията когато всички ленти са изключени

**Safety проверка:**

- На всеки 60 секунди системата проверява дали всички ленти са изключени
- Ако релето е ON и всички ленти са OFF, релето се изключва автоматично
- Това гарантира че релето никога не остава включено без нужда

### Транзиции

- 6 транзиционни ефекта (3 за включване, 3 за изключване): център↔краища, ляво→дясно
- Случайно избрани при всяко превключване (без „случайни LED“ wipe)
- Продължителност: 1 секунда
- При effect `fireworks` транзициите се пропускат — анимацията стартира веднага

### Ефекти (light mode)

- `normal` — солиден RGBW микс от channels
- `rainbow_static` — статична дъга по дължината на лентата
- `fireworks` — класически WLED-inspired fireworks (fade + случайни искри), анимиран в loop

### Димиране

- **Задържане на бутон** (само ленти 0, 1, 4): >250ms за старт; ~50 единици/с; редува нагоре/надолу; мигане при макс. Банята **няма** бутон за димиране.
- **MQTT `brightness`:** Всички ленти **включително баня** — плавно до целта; финален статус се публикува при **край на прехода**.
...
- **Димиране с бутон:** Лентата трябва да е ON; задръжте **>250ms** (само ленти 0, 1, 4 — не банята).
- **Димиране от приложение / MQTT:** Работи за **всички ленти включително банята**, когато лентата е ON; ако UI изостава, проверете дали topic за статус се обновява след края на fade.
- Проверете serial изхода за съобщения за димиране / плавна яркост
