# Мои бележки - SmartCamper 🚐

## 🎯 Какво правим

Система за управление на електрическата система на кемпера. Три части: мозък (Raspberry Pi), сензори (ESP32), dashboard (React).

## ✅ Направихме днес

### Backend (Express.js)

- ✅ Модулна структура - middleware, routes, socket в отделни папки
- ✅ Socket.io интеграция за real-time комуникация
- ✅ MQTT broker (Aedes) интеграция
- ✅ MQTT ↔ WebSocket bridge
- ✅ Health Check система - мониторинг на модулите
- ✅ Force Update система - форсиране на данни от ESP32
- ✅ Главна страница и health check endpoint
- ✅ 404 обработка
- ✅ Логване на заявки
- ✅ CORS middleware за development

### Frontend (React + Vite)

- ✅ React приложение с Vite
- ✅ WebSocket комуникация с backend
- ✅ Real-time обновления на сензорни данни
- ✅ useState и useEffect hooks
- ✅ Показва температура и влажност
- ✅ Статус на връзката (Онлайн/Офлайн)

### ESP32 Framework + Real Hardware

- ✅ Реален AM2301 (DHT22) сензор интеграция
- ✅ CommandHandler за force update команди
- ✅ Оптимизирана комуникация - праща само при промяна
- ✅ NetworkManager за WiFi управление
- ✅ MQTTManager за MQTT комуникация
- ✅ SensorManager за реални сензорни данни
- ✅ Config.h за централизирани настройки
- ✅ PlatformIO конфигурация
- ✅ Модулна архитектура (separation of concerns)

### Архитектура

- ✅ Backend като Gateway (ESP32 ↔ Backend ↔ Frontend)
- ✅ MQTT за ESP32 комуникация
- ✅ WebSocket за real-time данни
- ✅ HTTP fetch за бъдеща история (графики)
- ✅ Модулен и чист код (senior dev стил)

## 🔧 Какво работи

**Backend (порт 3000):**

- `http://localhost:3000` - главна страница
- `http://localhost:3000/health` - health check
- `ws://localhost:3000` - WebSocket сървър
- `mqtt://localhost:1883` - MQTT broker (Aedes)

**Frontend (порт 5173):**

- `http://localhost:5173` - React dashboard
- Real-time температура (променя се на всеки 3 сек)
- WebSocket connection status

**ESP32 Framework + Real Hardware:**

- Реални данни от AM2301 (DHT22) сензор
- Оптимизирано MQTT публикуване (само при промяна)
- Force Update команди от backend
- WiFi управление с автоматично преподключване
- CommandHandler за обработка на команди
- Универсална структура за всички модули

## 📁 Структура

```
smartCamper/
├── backend/              # Express сървър + Socket.io + MQTT
│   ├── server.js        # Конфигурация
│   ├── middleware/      # CORS, Logger
│   ├── routes/          # HTTP routes
│   ├── socket/          # WebSocket логика
│   └── mqtt/            # MQTT broker (Aedes)
├── frontend/            # React + Vite
│   └── src/
│       ├── App.jsx     # Главен компонент
│       └── App.css     # Стилове
├── esp32-modules/       # ESP32 Universal Framework
│   └── temperature-sensor/    # Конкретен модул с реални сензори
│       ├── platformio.ini
│       ├── include/
│       │   ├── SensorManager.h
│       │   └── CommandHandler.h
│       └── src/
│           ├── Config.h           # Настройки
│           ├── NetworkManager.*   # WiFi
│           ├── MQTTManager.*      # MQTT
│           ├── CommandHandler.cpp # Force Update команди
│           ├── SensorManager.cpp  # Реални AM2301 данни
│           └── main.cpp
├── PROJECT_LOG.md       # Детайлна документация
└── MY_NOTES.md         # Този файл
```

## 🚀 Следващо

- Тестване на ESP32 framework на реална платка
- Конфигуриране на WiFi/MQTT IP адреси
- Добавяне на реални сензори (DHT22, MPU6050)
- База данни за история
- Графики за исторически данни
- Контрол на релета/консуматори

## 💡 Полезно

```bash
# Backend
cd backend
npm start                    # Стартира сървъра на :3000

# Frontend
cd frontend
npm run dev                  # Стартира Vite dev server на :5173

# ESP32 (PlatformIO)
cd esp32-modules/temperature-sensor
pio run                      # Компилиране
pio run --target upload      # Качване на платката
pio device monitor           # Сериен монитор

# Убий процес на порт
lsof -ti:3000 | xargs kill -9
```

## 📚 Ключови концепции

**React:**

- `useState` - state management
- `useEffect` - side effects (WebSocket connection)
- `cleanup function` - disconnect при unmount

**Socket.io:**

- `io.on("connection")` - backend слуша
- `socket.emit()` - backend изпраща
- `socket.on()` - frontend слуша

**ESP32/C++:**

- `class` - групиране на данни и функции
- `static` - променливи запазват стойност между извикванията
- `WiFi.mode(WIFI_STA)` - Station режим за свързване към WiFi
- `mqttClient.publish()` - MQTT публикуване
- `millis()` - време в милисекунди от стартиране

**Архитектура:**

- MQTT за ESP32 комуникация ✅
- WebSocket за real-time данни ✅
- HTTP для история/конфигурация ✅
- Backend като Gateway ✅
- Модулна структура ✅

## 🎯 Важни решения

1. **НИКОГА директна комуникация Frontend ↔ ESP32**

   - Винаги през Backend като Gateway
   - Backend валидира, филтрира, добавя логика

2. **WebSocket за real-time, HTTP за история**

   - Socket.io за live сензорни данни
   - fetch() за история от база данни

3. **Модулна структура**

   - Всеки файл има ЕДНА отговорност
   - Лесно тестване и поддръжка

4. **Простота**
   - Чист и прост код
   - Без излишни усложнения

---

_Последно обновяване: 2025-10-02_
_Статус: Backend + Frontend + ESP32 Framework готови ✅_
